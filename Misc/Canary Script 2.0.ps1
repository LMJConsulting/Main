...
foreach ($path in $paths) {if (!(test-path $path.fullname)) {send-mailmessage -from "gsmith@lmjconsulting.com" -to "gsmith@lmjconsulting.com" -subject "Suspicious write activity on $env:COMPUTERNAME" -body "Canary at $env:COMPUTERNAME no longer exists" -credential ($cred) -usessl -smtpserver smtp.office365.com -port "587"} 
